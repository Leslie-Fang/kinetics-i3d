...
def train():
    ...
    rgb_saver = tf.compat.v1.train.import_meta_graph('./fine_tune_model_tf2_SGDoptimizer_lr3_lrDecay_20epoch/model.ckpt.meta')
    with tf.compat.v1.Session() as sess:
        ...
        # SGD epoch:5 lr:3 inference accuracy 3311/3776 = 0.8768
        # SGD epoch:1 lr:3 inference accuracy 2816/3776 = 0.745
        # optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=3).minimize(loss)

        # SGD epoch:5 lr:3 with lr decay(3.0, global_step, 100, 0.96) accuracy: 3389/3776 = 0.8975
        # SGD epoch:7 lr:3 with lr decay(3.0, global_step, 100, 0.96) accuracy: 3411/3776 = 0.9033
        # SGD epoch:9 lr:3 with lr decay(3.0, global_step, 100, 0.96) accuracy: 3439/3776 = 0.9107
        # SGD epoch:15 lr:3 with lr decay(3.0, global_step, 100, 0.96) accuracy: 3455/3776 = 0.9149
        # SGD epoch:20 lr:3 with lr decay(3.0, global_step, 100, 0.96) accuracy: 3447/3776 = 0.9128
        # SGD epoch:25 lr:3 with lr decay(3.0, global_step, 100, 0.96) accuracy: 3466/3776 = 0.9179
        global_step = tf.Variable(0, trainable=False)
        learning_rate = tf.compat.v1.train.exponential_decay(0.1, global_step, 100, 0.96)
        optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate).minimize(loss, global_step=global_step)

        # MomentumOptimizer: not convergent
        # optimizer = tf.compat.v1.train.MomentumOptimizer(learning_rate=3, momentum=0.9, name="MomentumOptimizer2").minimize(loss)
        sess.run(tf.compat.v1.global_variables_initializer())
        rgb_saver.restore(sess, "./fine_tune_model_tf2_SGDoptimizer_lr3_lrDecay_20epoch/model.ckpt")
        ...
        with tf.compat.v1.gfile.FastGFile("./fine_tune_model_tf2/pb/model.pb", mode='wb') as f:
